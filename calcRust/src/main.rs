...
impl<T> Stack<T> {
    ...
    fn peekf(&self) -> Option<&T> {
        //im not quite sure if the implementation of this functions
...
//***************************************************************************************************************************
fn postfixConvert(
    mut dataString: String,
    tempC: char,
    c: char,
    stack: &mut Stack<char>,
    listOfOps: &str,
) -> String {
    // debug
    ...
    if (c == ')') {
        ...
        if (stack.is_empty()) {
            //prevents code panicking from the else if statement below
...
//***************************************************************************************************************************
fn postfixCalculations(postData: &str, sStack: &mut Stack<String>, listOfOps: &str) -> f32 {
